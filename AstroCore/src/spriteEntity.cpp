...
void SpriteEntity::Draw(float deltaTime, Camera2D *camera)
{
    ...
    destRect = (Rectangle){floor(globalPos.x - (sprite->origin.x * transform.scale.x)), floor(globalPos.y - (sprite->origin.y * transform.scale.y)), sprite->spriteTexture.width * transform.scale.x, sprite->spriteTexture.height * transform.scale.y};
    DrawTexturePro(sprite->spriteTexture, srcRect, destRect, {floor(globalPos.x), floor(globalPos.y)}, GetGlobalRotation(), WHITE);
}
